...
def render_path(
    ...
    for i, (c2w, intrin) in enumerate(tqdm(zip(render_poses, intrinsics))):
        ...
        rays_d = rays_d.reshape(-1, 3)
        additional_pixel_information = {
            "ray_bending_latents": single_latent_code.reshape(1, intrin["ray_bending_latent_size"]).expand(
                height * width, intrin["ray_bending_latent_size"]),
        }
...
def create_nerf(args, autodecoder_variables=None, ignore_optimizer=False):
    ...
    if args.time_conditioned_baseline:
        ...
        if args.offsets_loss_weight > 0. or args.divergence_loss_weight > 0. or args.rigidity_loss_weight > 0.:
            raise RuntimeError(
                "Naive Baseline requires to turn off regularization losses since they only work with ray bending")
...
def get_full_resolution_intrinsics(args, dataset_extras):
    ...
    if dataset_extras["is_multiview"]:  # multi-view
        ...
        def _get_info(image_folder):
            ...
            imgnames = [f for f in sorted(os.listdir(imgdir)) if
        ...
        one_camera = {"height": height, "width": width, "focal_x": focal_x, "focal_y": focal_y, "center_x": center_x,
                      "center_y": center_y}
...
def main_function(args):
    ...
    min_point, max_point = determine_nerf_volume_extent(
        parallel_render, poses,
        [intrinsics[dataset_extras["imageid_to_viewid"][imageid]] for imageid in range(poses.shape[0])],
    ...
    print("get rays")
    rays = np.stack([get_rays_np(p, intrinsics[dataset_extras["imageid_to_viewid"][imageid]]) for imageid, p in
                     enumerate(poses[:, :3, :4])], 0)  # [N, ro+rd, H, W, 3]
    print("done, concats")
    ...
    for i in trange(start, N_iters):
        ...
        # reset autodecoder gradients to avoid wrong DeepSDF-style optimization. Note: this is only guaranteed to
        # work if the optimizer is Adam
        for latent in ray_bending_latents_list:
            latent.grad = None
        ...
        # first, test_images (if sampled image IDs give non-empty indicators). mask N_rays loss with indicators,
        # then take mean and loss backward with retain_graph=True. then None ray_bender (if existent) and Nerf grads
        ...
        ):  # in case images are very dark or very bright, need to keep network from initially building up so much
            # momentum that it kills the gradient
