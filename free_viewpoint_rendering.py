...
def _setup_nonrigid_nerf_network(results_folder, checkpoint="latest"):
    ...
    def render_convenient(
        ...
        if with_ray_bending:
            ...
            if "network_fine" in render_kwargs_test_:
                render_kwargs_test_[
                    "network_fine"].test_time_nonrigid_object_removal_threshold = backup_foreground_removal
        else:
...
def free_viewpoint_rendering(args):
    ...
    import tqdm
    for start_index in tqdm.tqdm(range(0, num_output_frames, frames_at_a_time),
                                 total=len(list(range(0, num_output_frames, frames_at_a_time)))):
        ...
        sublatents = [latents[0] for i in range(start_index, end_index)]
    ...
    if args.camera_path == "fixed":
        ...
        max_value = 1
        min_value = 0
        averaged_standard_devations = np.clip(averaged_standard_devations, a_max=max_value,
                                              a_min=min_value) / max_value  # cut off above max_value. result is normalized to [0,1]
        averaged_standard_devations = (255. * averaged_standard_devations).astype(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--camera_path",
        default='input_reconstruction',
        type=str,
        help='"input_reconstruction", "fixed". camera path to use for re-rendering. optionally, implement "spiral", '
             'see README.md',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='only used for "fixed" camera_path. groundtruth camera view index that is used for fixed-view '
             're-rendering. default is 0.',
    )
